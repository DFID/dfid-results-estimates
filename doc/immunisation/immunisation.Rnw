...
\Sexpr{dept %>% filter(department=="Multilateral" & gender=="Total" & year=="2015/21" & immunisation=="Immunised") %>% summarise(results/1000000) %>% unlist() %>% roundChoose(.,.1)}
million} children, saving \textbf{
\Sexpr{dept %>% filter(department=="Multilateral" & gender=="Total" & year=="2015/21" & immunisation=="Saved") %>% summarise(results/1000000) %>% unlist() %>% roundChoose(.,.1)}
million} lives. %
