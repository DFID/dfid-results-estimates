...
<<echo=FALSE>>=
loadd(wash_fragility_data)
loadd(wash_region_data)
loadd(wash_table)
@
...
\Sexpr{wash_table %>% filter(Department=="Total") %>% summarise(Total/1000000) %>% unlist()} million} people to gain access to clean water and/or better sanitation. %

\begin{figure}[htbp]
	\centering
<<wash_region_plot, echo=FALSE, fig.dim=c(8,6), out.width='0.8\\textwidth', fig.path='figs/'>>=
loadd(wash_region_plot)
plot(wash_region_plot)
@
...
From 2015 to 2020, Africa was the largest beneficiary of DFID Water Supply, Sanitation and Hygiene (WASH) programmes, with \Sexpr{wash_region_data %>% filter(region=="Africa") %>% summarise(results/1000000) %>% unlist() %>% roundChoose(.,.1)} million beneficiaries reached. %
DFID also reached over \Sexpr{wash_region_data %>% filter(region=="Middle East") %>% summarise(results/1000000) %>% unlist() %>% roundChoose(.,1)} million beneficiaries in the Middle East --- the majority of whom were in Syria (\Sexpr{wash_table %>% filter(Department=="Syria") %>% summarise(Total/1000000) %>% unlist() %>% roundChoose(.,.1)} million) --- and \Sexpr{wash_region_data %>% filter(region=="Asia") %>% summarise(results/1000000) %>% unlist() %>% roundChoose(.,.1)} million beneficiaries in Asia. %
A further \Sexpr{wash_region_data %>% filter(region=="Policy" | region=="Multilateral") %>% summarise(sum(perc)) %>% unlist() %>% roundChoose(.,1)}\% (\Sexpr{wash_region_data %>% filter(region=="Policy" | region=="Multilateral") %>% summarise(sum(results)/1000000) %>% unlist() %>% roundChoose(.,.1)} million beneficiaries) of DFID's WASH results were delivered via non-country specific programmes, non-region-specific programmes, and multilateral organisations. %

\begin{figure}[htbp]
	\centering
<<wash_fragility_plot, echo=FALSE, fig.dim=c(8,6), out.width='0.8\\textwidth', fig.path='figs/'>>=
loadd(wash_fragility_plot)
plot(wash_fragility_plot)
@
...
Most of the population reached by DFID WASH programs live in fragile states (\Sexpr{wash_fragility_data %>% filter(fragility=="Extremely Fragile" | fragility=="Fragile") %>% summarise(sum(results)/1000000) %>% unlist() %>% roundChoose(.,.1)} million beneficiaries), including \Sexpr{wash_fragility_data %>% filter(fragility=="Extremely Fragile") %>% summarise(sum(results)/1000000) %>% unlist() %>% roundChoose(.,.1)} million beneficiaries living in extremely fragile states. %

Of the results that have been disaggregated by gender from 2015 to 2020, DFID WASH programs reached \Sexpr{wash_table %>% filter(Department=="Total") %>% summarise(sum(Female)/1000000) %>% unlist() %>% roundChoose(.,.1)} million women. %
In 2019/20 \Sexpr{wash_table %>% filter(Department=="Total") %>% summarise(((sum(Female,Male)/Total)*100)) %>% unlist() %>% roundChoose(.,1, up=TRUE)}\% of our reported WASH results were disaggregated by gender. %
