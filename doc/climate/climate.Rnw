...
<<climate_spend_plot, include=FALSE>>=
vals= c(2909, 3641)
type= c("spend", "target")
dat <-  data.frame(vals, type)

ggplot(dat[1,], aes(x=type, y=vals)) +
  geom_bar(stat="identity", size=1, fill=gov_cols[10],  color=darken(gov_cols[10])) +
  geom_hline(aes(yintercept=dat[2,1]), color="red",  size=0.8, linetype="dashed") +
