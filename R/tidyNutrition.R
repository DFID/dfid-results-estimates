...
#'
#' This function parses data from the NUTRITION tab on the SDP templates and returns a long format table.
#' @param template SDP template for relevant indicator
#' @keywords names
#' @importFrom magrittr %>%
#' @examples formatNutritionDept(nutrition.raw)
#' @export
formatNutritionDept <- function(template){
  # read in bits of sheet for forecast and achieved

  forecast <- template %>% janitor::remove_empty(which = "rows") %>% dplyr::slice(5:11) %>% janitor::remove_empty(which = "cols") %>% as.data.frame()
  achieved <- template %>% janitor::remove_empty(which = "rows") %>% dplyr::slice(12:18) %>% janitor::remove_empty(which = "cols") %>% as.data.frame()

  # metadata
  metadata <- template %>% janitor::remove_empty(which = "rows") %>% dplyr::slice(3) %>% janitor::remove_empty(which = "cols")
  ...
  forecast %<>%
    ...
    dplyr::mutate(nutrition_intensity= ifelse(split=="total","all", ifelse(split=="disabled",NA, split))) %>% # add in intensity info
  ...
  achieved %<>%
    ...
    dplyr::mutate(split=tolower(split)) %>% dplyr::mutate(gender=tolower(gender)) %>%
    dplyr::mutate(nutrition_intensity= ifelse(split=="total","all", ifelse(split=="disabled",NA, split))) %>% # add in intensity info
    dplyr::mutate(disability = ifelse(split=="disabled","disabled",NA)) %>% # add disabled col
    dplyr::mutate(result=ifelse(result<0,0,result)) %>% # clean negative numbers
    dplyr::select(-split)
