...
#' @export
#'
plotWASHFragility <- function(data) {
  ...
  wash_frag_plot  <-
    ...
    labs(
      ...
      subtitle = "Percentage of WASH Results by Fragility Level\n\n",
    ...
    scale_y_continuous(breaks=seq(0, roundChoose(max(wash_plot_data$perc),10,TRUE), by = 10))


  ### comment in if not using drake
  pdf(file="figs/wash_fragility_plot.pdf",
      width=8,
      height=6,
      pointsize=12)
  print(wash_frag_plot)
  dev.off()
}
