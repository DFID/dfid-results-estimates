...
#' @export
formatWashDept <- function(dat){
# read in bits of sheet for forecast and achieved
x <- as.data.frame(dat[2])
y <-  cbind(result_split=c(NA,NA,unlist(df[3])), as.data.frame(df[29])) %>%
  dplyr::mutate_at("result_split", as.character) # pull the bits out of the table soup
...
x %<>%
  ...
  dplyr::mutate(wash_intervention= ifelse(result_split=="total","all", ifelse(result_split=="disabled",NA, result_split))) %>% #add
