#' Format NUTRITION  tab
#'
#' This function takes output from jailbreakr for the NUTRITION tab on the SDP templates and returns a long format table.
#' @param dat List of tiny little matrices extracted from sector templates with jailbreakr
#' @keywords names
#' @importFrom magrittr %>%
#' @examples formatNutritionDept(nutrition.raw)
#' @export
formatNutritionDept <- function(dat){
  # read in bits of sheet for forecast and achieved
  x <- as.data.frame(dat[2])
  y <-  cbind(result_split=c(NA,NA,unlist(df[3])), as.data.frame(df[29])) %>% dplyr::mutate_at("result_split", as.character) # pull the bits out of the table soup
  ...
  x %<>%
    ...
    rename_at(vars(starts_with("Total")), # rename these cols
              funs(str_replace(., "Total", "2015/21"))) %>%
    ...
    unnest() %>%
    dplyr::mutate(year=recode(year, Total="2015/21")) %>%  # reporting period
    dplyr::mutate(achieved_forecast="forecast") %>%
    dplyr::mutate(result_split= replace(result_split, str_detect(result_split, "disabled"), "disabled")) %>%
    dplyr::mutate(result_split=recode(result_split, `Forecast Results - Total`="total")) %>%
    dplyr::mutate(result_split=tolower(result_split)) %>% dplyr::mutate(gender=tolower(gender)) %>%
    dplyr::mutate(nutrition_intensity= ifelse(result_split=="total","all", ifelse(result_split=="disabled",NA, result_split))) %>% #add
  ...
  y %<>%
    ...
    dplyr::mutate(result_split= replace(result_split, stringr::str_detect(result_split, "disabled"), "disabled")) %>%
    dplyr::mutate(result_split = dplyr::recode(result_split, `Achieved  Results - Total`="total")) %>%
    dplyr::mutate(result_split=tolower(result_split)) %>% dplyr::mutate(gender=tolower(gender)) %>%
    dplyr::mutate(nutrition_intensity= ifelse(result_split=="total","all", ifelse(result_split=="disabled",NA, result_split))) %>% #add
    dplyr::mutate(disability = ifelse(result_split=="disabled","disabled",NA)) %>% # add diabled col
    dplyr::mutate(result=ifelse(result<0,0,result)) %>% # clean negative numbers
