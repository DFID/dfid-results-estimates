...
#'
#' This function parses data from the EDUCATION tab on the SDP templates and returns a long format table.
#' @param template SDP template for relevant indicator
#' @keywords names
#' @importFrom magrittr %>%
#' @examples formatEducationDept(education.raw)
#' @export
formatEducationDept <- function(template){
  ...
  forecast %<>%
    ...
    dplyr::mutate(education_level= ifelse(split=="total","all", ifelse(split=="disabled",NA, split))) %>% #add
