#' Format WASH tab
#'
#' This function parses data from the WASH tab on the SDP templates and returns a long format table.
#' @param template SDP template for relevant indicator
#' @keywords names
#' @importFrom magrittr %>%
#' @examples formatWashDept(wash.raw)
#' @export
formatWashDept <- function(template){
  # read in bits of sheet for forecast and achieved

  forecast <- template %>% janitor::remove_empty(which = "rows") %>% dplyr::slice(5:10) %>% janitor::remove_empty(which = "cols") %>% as.data.frame()
  achieved <- template %>% janitor::remove_empty(which = "rows") %>% dplyr::slice(11:17) %>% janitor::remove_empty(which = "cols") %>% as.data.frame()
  ...
  forecast %<>%
    ...
    dplyr::mutate(wash_intervention= ifelse(split=="total","all", ifelse(split=="disabled",NA, split))) %>% #add wash intervention
