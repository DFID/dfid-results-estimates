...
#'
#' This function takes output from jailbreakr for the HUMANITARIAN tab on the SDP templates and returns a long format table.
...
#' @export
formatHumanitarianDept <- function(df){
# read in bits of sheet for forecast and achieved
x <- as.data.frame(df[2])
y <-  cbind(result_split=c(NA,NA,unlist(df[3])), as.data.frame(df[21])) %>% dplyr::mutate_at("result_split", as.character) # pull the bits out of the table soup
...
x %<>%
  ...
  dplyr::mutate(result_split=dplyr::recode(result_split, `Forecast Results - Total`="total")) %>%
